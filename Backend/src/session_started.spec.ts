  ...
  test('creating BallTransaction persists player.start_time = delivered_time + 30s', async () => {
    ...
    if (!bay)
      bay = await prisma.bay.create({
        data: { bay_number: '1', status: 'Available' },
      });
    ...
    const player = await prisma.player.create({
      data: {
        ...
        creator: { connect: { employee_id: emp.employee_id } },
      },
    });
    const assignment = await prisma.bayAssignment.create({
      data: {
        ...
        dispatcher: { connect: { employee_id: emp.employee_id } },
    ...
    // cleanup
    await prisma.ballTransaction
      .delete({ where: { transaction_id: tx.transaction_id } })
      .catch(() => null);
    await prisma.bayAssignment
      .delete({ where: { assignment_id: assignment.assignment_id } })
      .catch(() => null);
    await prisma.player
      .delete({ where: { player_id: player.player_id } })
      .catch(() => null);
    await prisma.bay
      .delete({ where: { bay_id: bay.bay_id } })
      .catch(() => null);
    await prisma.employee
      .delete({ where: { employee_id: emp.employee_id } })
      .catch(() => null);
  });
