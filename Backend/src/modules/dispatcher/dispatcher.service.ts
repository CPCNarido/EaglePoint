...
export class DispatcherService {
  ...
  async getOverview() {
    ...
    const bays = baysRaw.map((b) => {
      ...
      const playerName =
        assignment?.player?.nickname ?? assignment?.player?.full_name ?? null;
      const endTime =
    ...
    const maintenanceBays = bays.filter(
      (x) => String(x.originalStatus) === 'Maintenance',
    ).length;
    const occupiedBays = bays.filter(
      (x) => String(x.status) === 'Occupied',
    ).length;
    ...
    try {
      ...
      staffOnDuty = await (this.prisma as any).attendance.count({
        where: { date: dateBucket, clock_in: { not: null }, clock_out: null },
      });
      Logger.log(
        `Dispatcher overview computed staffOnDuty=${staffOnDuty} using attendance table for date=${dateBucket.toISOString()}`,
        'DispatcherService',
      );
    } catch (e) {
      // Fallback to total employees if attendance table isn't available or an error occurs
      try {
        staffOnDuty = await this.prisma.employee.count();
        Logger.log(
          `Dispatcher overview falling back to employee count=${staffOnDuty} due to attendance error`,
          'DispatcherService',
        );
      } catch {
        staffOnDuty = 0;
    ...
    try {
      ...
      try {
        const emps = await this.prisma.employee.findMany({
          select: { employee_id: true, full_name: true, username: true },
        });
