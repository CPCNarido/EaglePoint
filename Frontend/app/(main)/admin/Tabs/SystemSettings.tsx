...
export default function SystemSettings() {
  ...
  const notifTimer = React.useRef<number | null>(null);
  ...
  const [totalAvailableBays, setTotalAvailableBays] = useState('45');
  const [teeInterval, setTeeInterval] = useState('10');
  const [bucketWarning, setBucketWarning] = useState('5');

  // General settings
  const [siteName, setSiteName] = useState('Eagle Point');
  const [currencySymbol, setCurrencySymbol] = useState('₱');
  
  const [enableReservations, setEnableReservations] = useState<boolean>(true);
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      // General
      if (data.siteName || data.siteTitle) setSiteName(String(data.siteName ?? data.siteTitle ?? 'Eagle Point'));
  ...
  const saveOperational = async () => {
    ...
    // ask for reconfirmation before saving
    const payload: any = {
  ...
  try { if (typeof window !== 'undefined' && window.dispatchEvent) window.dispatchEvent(new Event('overview:updated')); } catch {}
      ...
      // use window.setTimeout to get a number type in TS
  ...
  // reset/clear timers when component unmounts
  useEffect(() => {
    return () => {
      if (notifTimer.current) {
        clearTimeout(notifTimer.current as unknown as number);
        notifTimer.current = null;
      }
    };
  }, []);
    ...
    <View style={styles.rootContainer}>
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={[tw.flex1, { backgroundColor: '#F6F6F2' }]}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            <Text style={styles.label}>Timed Session Rate (₱/hour)</Text>
            <TextInput value={timedRate} onChangeText={setTimedRate} style={styles.input} keyboardType='number-pad' />
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Open Time Rate (₱/hour)</Text>
  ...
  {/* Confirm Changes Modal */}
      ...
      {/* Saved Notification (transient) rendered outside the ScrollView so it floats */}
      {showSavedNotification && (
        <Pressable
          style={styles.notifOverlay}
          onPress={() => {
            // reset auto-dismiss timer when user touches the notification
            if (notifTimer.current) {
              clearTimeout(notifTimer.current as unknown as number);
            }
            notifTimer.current = window.setTimeout(() => {
...
const styles = StyleSheet.create({
  ...
  /* Confirm modal styles (matches provided design) */
